...
exports.create = async (req, res) => {
    ...
    try {
        ...
        const data = await DepartmentOrganization.create({
...
exports.selectAllById = async (req, res) => {
    const { id } = req.params;
    try {
        const data = await DepartmentOrganization.findAndCountAll({where:{id}});
        res.status(200).json(data)
    } catch (error) {
        return res.status(500).json({ message: error.message });
    }
...
exports.selectById = async (req, res) => {
    try {
        ...
        if (!departmentOrganization) {
            return res.status(404).json({ error: 'Department organization not found' });
        }

        res.status(200).json(departmentOrganization);
    } catch (error) {
        console.error(error.message);
        res.status(500).json({ error: 'Failed to retrieve the department organization' });
    }
}
...
exports.delete = async (req, res) => {
    try {
        ...
        await data.destroy()
        res.status(200).json({ message: 'delete department organization successfully' })
    } catch (error) {
        return res.status(500).json({ message: error.message });
    }
}